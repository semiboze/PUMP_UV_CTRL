...
void measurePeakCurrent() {
  ...
  if (analog_cnt >= (1500 / TIMER_INTERVAL_MS)) {
    ...
    if (current_reading_max > PUMP_CURRENT_THRESHOLD_DEFAULT) { // 
      ...
      if (pumpState == STATE_RUNNING && !pumpStartupError && !pumpStartupOk) {
        ...
        // フェーズ3：2次上昇待ち（吸い上げて負荷が乗るのを待つ）
        else if (startupPhase == STARTUP_WAIT_RISE) {

          // 上昇判定：ベースライン平均 + Δ を超えたら上昇とみなす
          // min基準にしたいなら baselineMin + RISE_DELTA に変える
          int riseLine = baselineAvg + RISE_DELTA;

          if (lastCurrentPeak >= riseLine) {
            riseConsecutive++;
          } else {
            // ちょっと落ちたらリセット（ノイズで誤判定しない）
            riseConsecutive = 0;
          }

          if (riseConsecutive >= RISE_CONSECUTIVE) {
            suctionRiseDetected = true;
            pumpStartupOk = true;                 // ← ここで「吸えた」確定
            startupPhase = STARTUP_DONE_OK;

            PU_DEBUG_PRINTLN("Suction OK: 2nd rise detected.");
          }

          // タイムアウトは updateSystemState 側で止める（責務分離）
        }
      }
      //====================================================
      // ▲▲▲ ここまで追加 ▲▲▲
