...
void updateUvSystemState() {
  ...
  if (pumpState == STATE_STOPPED || uvLampState == STATE_RUNNING) {
      // uvLampState = STATE_STOPPED;
      // DEBUG_PRINTLN("UV lamp stopped along with the pump.");
  }
}
...
void runStartupLedSequence(int lampCount) {
  ...
  // --- シーケンス開始 ---
  DEBUG_PRINTLN("Starting LED self-check sequence...");

  // 1. 固定LEDを順番に点灯・消灯
  for(int i = 0; i < numStaticLEDs; i++) {
    digitalWrite(staticLEDs[i], HIGH);
    delay(1);
    //DEBUG_PRINTLN(String(staticLEDNames[i]) + " ON");
  }
  delay(main_interval);  
  for(int i = 0; i < numStaticLEDs; i++) {
    digitalWrite(staticLEDs[i], LOW);
    delay(1);
    //DEBUG_PRINTLN(String(staticLEDNames[i]) + " OFF");
  }
  ...
  DEBUG_PRINTLN("LED self-check sequence complete.");
}